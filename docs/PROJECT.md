...
| 31 | Artifact tables have no horizontal scroll and no resizable columns — long content causes rightmost columns (including the delete button) to be cut off and unreachable | Open | Artifact tables (Company, Role, Candidate, Interviewer — likely all) use fixed or content-driven column widths with no `overflow-x: auto` wrapper. When a text artifact with a long description is added, column 1 expands and pushes columns 3/4 (DATE ADDED, action buttons) off-screen. Users cannot delete such rows. Fix requires: (a) wrapping the table in an `overflow-x-auto` container so horizontal scrolling is available, and (b) optionally adding resizable columns (e.g. `resize: horizontal` on `<th>` or a drag-handle pattern). Affects `ArtifactsSection.tsx` and any other artifact table components. |
