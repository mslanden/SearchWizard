...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | Open — Awaiting SQL | `user_id` column is UUID type — corrected SQL (no cast) provided in `SETUP.md` migration #6. User must drop existing policies and re-run with corrected statements: `WITH CHECK (auth.uid() = user_id)` |
