...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | Fixed via SQL | Pre-existing "Users can manage own process artifacts" ALL policy used a complex JOIN through `interviewers`/`projects` tables that conflicted with the INSERT. Fix: dropped all 4 policies, recreated 3 simple ones checking `auth.uid() = user_id` directly on `process_artifacts` |
