...
| 32 | ARTIFACTS count in Candidates table always shows 0 — does not increment on add or decrement on delete | Open | The ARTIFACTS column in the Candidates table displays 0 for all candidates even when artifacts exist (confirmed: candidate has 2 artifacts in their profile popup but table shows 0). The `artifacts_count` column was added to the `candidates` table via SQL (Bug #12 fix), but the count is never updated. Required behavior: increment by 1 after each successful artifact insert; decrement by 1 after each successful artifact delete. Fix requires either Supabase triggers (`AFTER INSERT` and `AFTER DELETE` on `candidate_artifacts` → update `candidates.artifacts_count`) or explicit update calls in `candidateApi.js` after each insert/delete. Same issue likely affects the Interviewers table ARTIFACTS column (`process_artifacts` → `interviewers.artifacts_count`). |
