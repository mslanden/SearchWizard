...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | **SQL applied (Feb 2026) — policies verified; pending staging test** | `user_id` IS correctly sent in the insert (`interviewerApi.js` line 65). Root cause: conflicting/malformed Supabase RLS policies on `process_artifacts`. Fix: dropped all existing policies and recreated 3 clean policies (`auth.uid() = user_id` for INSERT/SELECT/DELETE) + `ALTER TABLE process_artifacts ENABLE ROW LEVEL SECURITY`. Policy audit confirmed all 3 policies now present and correctly structured (INSERT uses `WITH CHECK`, SELECT/DELETE use `USING`). Verify by uploading an interviewer artifact on staging — no RLS error should appear. |
