...
| 37 | Clicking "Generate New" briefly flashes the old V2 popup before the V3 popup appears | **Fixed (Mar 2026)** | `loading` state initialised as `true` in `useDocumentGenerationV3.js` so spinner shows immediately on mount, preventing empty-state flash. |
| 38 | "Generate New Document" popup remains open after user clicks "Generate by Magic" — should close immediately and show only the floating generating card | **Fixed (Mar 2026)** | `setIsGenerating(true)` moved before `await callGenerateV3()` in `handleGenerateMagic` and `handleGenerateFromPreview` — popup transitions to floating card immediately on click. |
