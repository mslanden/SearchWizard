...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | Open — Reopened | RLS policies were added (migration #6) but error persists. Possible causes: (a) `user_id` in `process_artifacts` is TEXT not UUID so `auth.uid() = user_id::uuid` cast fails silently at insert, (b) `addProcessArtifact` in `interviewerApi.js` may not be sending `user_id` in the insert record, or (c) a pre-existing conflicting policy is blocking the insert |
...
| 28 | Golden Examples table: TYPE shown as colored bubble; FEATURES column has no value | Open | (a) The TYPE column renders as a pill/badge (blue background, rounded) — it should be plain text to match all other artifact tables. (b) The FEATURES column (showing "Template" badge) provides no useful information to the user and should be removed entirely from the Golden Examples table |
