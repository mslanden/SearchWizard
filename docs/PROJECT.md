...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | **Requires SQL** — user must run in Supabase dashboard | `user_id` IS correctly sent in the insert (`interviewerApi.js` line 65). The issue is a conflicting Supabase RLS policy. Code confirmed clean. To fix: run the SQL in `docs/SETUP.md` migration #6 (drop ALL existing `process_artifacts` policies, then recreate the 3 simple ones with `auth.uid() = user_id`, then `ALTER TABLE process_artifacts ENABLE ROW LEVEL SECURITY`). |
