...
| 18 | Interviewer artifact upload fails with "new row violates row-level security policy" | **Reopened** — SQL fix did not resolve | Pre-existing "Users can manage own process artifacts" ALL policy used a complex JOIN through `interviewers`/`projects` tables conflicting with the INSERT. A SQL fix was applied (drop all policies, recreate 3 simple ones with `auth.uid() = user_id`), but the error persists as of Feb 25 2026. Next step: audit the current policies live in Supabase (`SELECT policyname, cmd, qual, with_check FROM pg_policies WHERE tablename = 'process_artifacts'`), confirm whether the fix was applied, and whether the `user_id` column on `process_artifacts` is actually populated by the insert in `interviewerApi.js`. |
...
| 34 | Golden Examples popup: "View File" button returns 400 — "querystring must have required property 'token'" | Open | Clicking the "View File" icon (first of three icons in column 5 of the Golden Examples table) opens a URL that returns `{"statusCode":"400","error":"Error","message":"querystring must have required property 'token'"}`. This is a Supabase Storage error: the file URL being used is a direct storage path rather than a signed URL, so the required `token` query parameter is absent. Fix: the "View File" handler must call `supabase.storage.from('golden-examples').createSignedUrl(filePath, 60)` to generate a time-limited signed URL before opening it, rather than constructing or using a raw storage URL. Check the `GoldenExamplesPopup.jsx` handler for the first icon button. |
| 33 | Uploading a Markdown (.md) file as a Candidate artifact fails — "mime type application/octet-stream is not supported" | Open | Browsers that don't recognise the `.md` extension send it with MIME type `application/octet-stream` (generic binary fallback) rather than `text/markdown` or `text/plain`. The backend file-type whitelist in `api.py` (or equivalent validation layer) does not include `application/octet-stream`, so the upload is rejected. Fix options: (a) add `application/octet-stream` to the allowed MIME type list — broad, may allow unintended file types; or (b) perform extension-based fallback: if MIME is `application/octet-stream` and extension is `.md`, `.txt`, etc., treat as plain text. Markdown is a common artifact format (e.g. candidate profiles, bios) so it should be supported. |
| 32 | ARTIFACTS count in Candidates table always shows 0 — does not reflect actual artifact count | Open | The ARTIFACTS column in the Candidates table displays 0 for all candidates even when artifacts exist (confirmed: candidate has 2 artifacts in their profile popup but table shows 0). The `artifacts_count` column was added to the `candidates` table via SQL (Bug #12 fix), but the count is never incremented. Likely causes: (a) `addCandidateArtifact` in `candidateApi.js` does not update `artifacts_count` after insert, and/or (b) the optimistic update in `page.tsx` after artifact upload does not increment the candidate's `artifactsCount`. Fix requires either a Supabase trigger (`AFTER INSERT ON candidate_artifacts` → increment `candidates.artifacts_count`) or an explicit update call in `candidateApi.js` after each successful artifact insert. Same issue likely affects the Interviewers table ARTIFACTS column. |
| 31 | Artifact tables: no visible scrollbar when content overflows — rightmost columns (including delete button) are cut off and not obviously reachable | Open | Horizontal scrolling technically works (user can grab and drag the table) but no scrollbar appears, so the affordance is invisible. When a text artifact with a long body is added, column 1 expands and pushes DATE ADDED and action columns off-screen with no indication they exist. Root cause: the table container likely has `overflow-x: auto` but the scrollbar is suppressed (e.g. via webkit scrollbar CSS or Tailwind `scrollbar-hide`). Fix: make the horizontal scrollbar always visible on the table container (remove `scrollbar-hide` if present, or add `scrollbar` / `overflow-x: scroll`). Optionally add resizable columns. Affects `ArtifactsSection.tsx` and any other artifact table components. |
| 30 | Adding a Company artifact via URL input fails — "Source URL is required for URL artifacts" | Open | Backend rejects the upload with "Source URL is required for URL artifacts". Frontend `useEnhancedArtifactUpload.js` stores the URL in `sourceUrl` (camelCase) and includes it in `uploadPayload.sourceUrl`, but the backend artifact handler likely expects `source_url` (snake_case). The page then crashes to "Error Loading Project" because the error propagates to `handleError`. Note: a simultaneous green "Company artifact uploaded successfully" toast appears, which is from a prior file upload in the same session — the crash relates to the URL upload only. Fix requires aligning the key name between frontend payload and backend validation (`source_url` vs `sourceUrl`) in `storageApi.js` or `api.py`. |
