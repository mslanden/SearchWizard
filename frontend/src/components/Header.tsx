...
import { Cog6ToothIcon, UserCircleIcon, ShieldCheckIcon, QuestionMarkCircleIcon } from '@heroicons/react/24/outline';
...
const Header: React.FC = () => {
  ...
  return (
    <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-dark-border bg-white dark:bg-dark-bg-secondary transition-colors">
      ...
      <nav className="flex items-center space-x-1" aria-label="User navigation">
        {/* Top-level text nav links */}
        <Link
          href="/"
          className="text-sm font-medium text-gray-700 dark:text-dark-text-secondary hover:text-gray-900 dark:hover:text-dark-text px-3 py-1.5 rounded transition-colors"
        >
          Projects
        </Link>
        <Link
        ...
        {/* Divider */}
        <div className="h-5 border-r border-gray-200 dark:border-dark-border mx-1" aria-hidden="true" />
        ...
        <Link
          ...
          aria-label="Help"
        >
          <QuestionMarkCircleIcon className="w-6 h-6 text-gray-700 dark:text-dark-text-secondary" />
        </Link>
